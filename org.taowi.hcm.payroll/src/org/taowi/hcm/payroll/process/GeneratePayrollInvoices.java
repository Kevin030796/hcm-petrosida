...
public class GeneratePayrollInvoices  extends SvrProcess {
	 ...
	 * Return getHR_ConceptName
	  ...
	  * @param c_bpartner_id
	 */	
	public static String getHC_ConceptName(String trx_name, int hr_concept_id) {
		
		String sConsulta = "SELECT value FROM hc_paycomponent WHERE  hc_paycomponent_id = ? ";
	...
	protected String doIt() throws Exception
	{
		...
		/* Main query */
		List<MHCMovement> movements = new Query(getCtx(), MHCMovement.Table_Name, whereClause.toString(), get_TrxName())
		...
		MEmployee emp = null;
		int startLine = 0;
		for (MHCMovement movement : movements) {
			if (oldBP != movement.getHC_Employee_ID()) {
				if (invoice != null) {
				...
				MHCPayrollProcess process = (MHCPayrollProcess) movement.getHC_PayrollProcess();
				emp = (MEmployee) movement.getHC_Employee();
				//MEmployee em = MEmployee.getActiveEmployee(getCtx(), movement.getC_BPartner_ID(), get_TrxName());
				invoice = new MInvoice(getCtx(), 0, get_TrxName());
				//TODO set employee id in invoice
				...
				oldEmp = movement.getHC_Employee_ID();
			}
			//MHRConcept concept = (MHRConcept) movement.getHR_Concept();
			MHCPayComponent concept = (MHCPayComponent) movement.getHC_PayComponent();
			int chargeID = concept.get_ValueAsInt("C_Charge_ID");
			
			String HR_ConceptName = getHC_ConceptName(get_TrxName(), movement.getHC_PayComponent_ID());
			...
			//if (MHRMovement.ACCOUNTSIGN_Credit.equals(movement.getAccountSign()))
				//line.setPrice(movement.getHC_AmtValue().negate());
			//else
				//line.setPrice(movement.getHC_AmtValue());
			
			//TODO what must be set
